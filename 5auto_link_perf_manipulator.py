...
class Net_Topology(Topo):
    ...
    def build(self, k = 2):
        ...
        self.addLink(h1, s1, cls=TCLink, bw=int(sys.argv[1]), delay=int(sys.argv[2]), loss=int(sys.argv[3]), jitter=int(sys.argv[4]))
        #link with 100 Mbps BW, 50ms delay, 25% packet loss and 2ms jitter
        self.addLink(h2, s2, cls=TCLink, bw=int(sys.argv[5]), delay=int(sys.argv[6]), loss=int(sys.argv[7]), jitter=int(sys.argv[8]))
