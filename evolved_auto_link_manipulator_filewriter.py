...
def run():
    ...
    info(net['h1'].cmd('ping -c 50 %s' %net['r0'].IP() + '| tee -a test_output.txt'))
    info(net['h1_2'].cmd('ping -c 50 %s' %net['r0'].IP() + '| tee -a test_output.txt'))
    info(net['h1_3'].cmd('ping -c 50 %s' %net['r0'].IP() + '| tee -a test_output.txt'))
    info(net['h2'].cmd('ping -c 50 %s' %net['r0'].IP() + '| tee -a test_output.txt'))
    info(net['h2_2'].cmd('ping -c 50 %s' %net['r0'].IP() + '| tee -a test_output.txt'))
    info(net['h2_3'].cmd('ping -c 50 %s' %net['r0'].IP() + '| tee -a test_output.txt'))
    
    ###Test Link Bandwidths between hosts and router###
    info(net['h1'].cmd('iperf %s' %net['h1'].IP() + '%s' %net['r0'] + '| tee -a test_output.txt'))
    info(net['h1_2'].cmd('iperf %s' %net['h1_2'].IP() + '%s' %net['r0'] + '| tee -a test_output.txt'))
    info(net['h1_3'].cmd('iperf %s' %net['h1_3'].IP() + '%s' %net['r0'] + '| tee -a test_output.txt'))
    info(net['h2'].cmd('iperf %s' %net['h2'].IP() + '%s' %net['r0'] + '| tee -a test_output.txt'))
    info(net['h2_2'].cmd('iperf %s' %net['h2_2'].IP() + '%s' %net['r0'] + '| tee -a test_output.txt'))
    info(net['h2_3'].cmd('iperf %s' %net['h2_3'].IP() + '%s' %net['r0'] + '| tee -a test_output.txt'))
